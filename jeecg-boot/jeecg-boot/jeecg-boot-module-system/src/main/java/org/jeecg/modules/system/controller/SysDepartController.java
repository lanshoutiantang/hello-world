...
//	@RequestParam(name="id",required=true) String id
	public Result getOrgLength(@RequestParam(name="parentId",required=true) String parentId)  throws Exception {
//		String parentId = sysDepart.getParentId();
		System.out.println(parentId);
		String orgLength = sysDepartService.getOrgLength(parentId);
		System.out.println(orgLength);
		/* 延时1.5秒，模拟网慢堵塞真实感 */
		Thread.sleep(500);
		return Result.OK(Integer.parseInt(orgLength));
	}
...
/*	@RequestMapping(value = "/add", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/add", method = RequestMethod.POST)
	@CacheEvict(value= {CacheConstant.SYS_DEPARTS_CACHE,CacheConstant.SYS_DEPART_IDS_CACHE}, allEntries=true)
	public Result<SysDepart> add(@RequestBody JSONObject params, HttpServletRequest request) {
		Result<SysDepart> result = new Result<SysDepart>();
		String departID = params.getString("departID");
		JSONObject formData = params.getJSONObject("formData");
		SysDepart sysDepart = JSON.toJavaObject(formData, SysDepart.class);
		String username = JwtUtil.getUserNameByToken(request);
		try {
			sysDepart.setCreateBy(username);
			sysDepartService.saveDepartData(sysDepart, username,departID);
...
/*	//@RequiresRoles({"admin"})
	...
	public Result<SysDepart> deleteBatch(@RequestParam(name = "ids", required = true) String ids) {
		...
		//根据idList查询数据库中是否有下级
		List<String> idList = sysDepartService.getIdList(id);


		if (ids == null || "".equals(ids.trim())) {
			result.error500("参数不正确！");
		}else if (idList.size() != 0){
			result.error500("所选结点下有子结点，删除失败！！");
		} else{
			this.sysDepartService.deleteBatchWithChildren(Arrays.asList(ids.split(",")));
			result.success("删除成功!");
		}
...
//	public String getOrgCode(@RequestParam("pId") String pId){
//		String parentOrgCode = sysDepartService.getParentOrgCode(pId);
//		String orgCode = null;
//		String chuName = sysDepartService.getDepartName(pId);
//		System.out.println("============" + chuName);
//		if (parentOrgCode.length() == 9){
//			orgCode = "chuOrgCode";
//		}else if (parentOrgCode.length() == 12){
//			//根据pid查找处对应的是哪个处
//			orgCode = sysDepartService.getOrgCode(chuName);
...
//	}
	...
	@RequestMapping("/getOrgCode")
	public SysCategory getOrgCode(@RequestParam("pId") String parentID){
		SysCategory sysCategory = new SysCategory();
		//如果flag为0 则表示采购线 如果为 1 则表示业务区
		int flag ;
		String pid = null;
		//根据id查询sys_category中的code
		//如果code中包含“A01” 则flag = 0 表示业务区代码
		String code = sysDepartService.getCodeById(parentID);
		System.out.println("code : " + code);
		if ("A01".equals(code.substring(0,3))){
			flag = 0;
		}else {
			flag = 1;
		}
		...
		if (flag == 0 ){
			System.out.println("进入采购线...");
			//采购线
			if (parentID == null || "".equals(parentID)){
				pid = sysDepartService.getCaigouxianId("0");
			}else {
				pid = sysDepartService.getParentIdByCode(parentID);
			}
			System.out.println("pid : " + pid);

		}else if (flag == 1){
			System.out.println("进入业务区...");
			//业务区
			String parentOrgCode = sysDepartService.getParentOrgCode(parentID);
			System.out.println("parentID: " + parentID);
			String departName = sysDepartService.getDepartName(parentID);
			System.out.println(departName);
			if (parentOrgCode.length() == 9){
				//pid = 0 根据分类字典查找所有的处类
				pid = sysDepartService.getCaigouxianId("0");
			}else if (parentOrgCode.length() > 9){
				//根据处的名字查询sys_category表中对应的id
				pid = sysDepartService.getParentId(departName);
			}else {
				pid = "shibai";
			}
		}
