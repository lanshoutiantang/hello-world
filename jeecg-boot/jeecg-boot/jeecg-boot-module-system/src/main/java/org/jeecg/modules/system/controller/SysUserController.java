...
public class SysUserController {
     ...
     * @param sysUser
     */
    @RequestMapping(value = "/userMoExportXls")
    public ModelAndView userMoExportXls(SysUser sysUser,HttpServletRequest request) {
        // Step.1 组装查询条件
        QueryWrapper<SysUser> queryWrapper = QueryGenerator.initQueryWrapper(sysUser, request.getParameterMap());
        //Step.2 AutoPoi 导出Excel
        ModelAndView mv = new ModelAndView(new JeecgEntityExcelView());
...
//        List<SysUser> sysUserList = new ArrayList();
        ...
        sysUser1.setUsername("请输入登陆账号 注：必选");
        sysUser1.setRealname("请输入真实姓名 注：必选");
//        sysUser1.setStatus("请输入状态 注：默认为正常");
//        sysUser1.setDelFlag("请输入删除状态 注：默认为正常");
        sysUser1.setWorkNo("请输入工号 注：必选");
//        sysUser1.setUserIdentity();
//        sysUser1.setDepartIds();
        pageList.add(sysUser1);
        //导出文件名称
        mv.addObject(NormalExcelConstants.FILE_NAME, "用户列表");
        mv.addObject(NormalExcelConstants.CLASS, SysUser.class);
        LoginUser user = (LoginUser) SecurityUtils.getSubject().getPrincipal();
    ...
    /**
     * 用户登录系统需要判断密码是否为‘123456’，如果是前端跳转到修改密码页面
     * @return
     */
    @RequestMapping(value = "/firstUpdatePassword", method = RequestMethod.GET)
    public String[] firstUpdatePassword(){
        LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
        String username = sysUser.getUsername();
        String password ="123456";
        String pp = sysUserService.firstPassword(username,password);
        String[] bb={username,pp};
...
//     */
//    @RequestMapping(value = "/firstUpdatePassword", method = RequestMethod.GET)
//    public Result<?> firstUpdatePassword(@RequestBody SysUser sysUser){
