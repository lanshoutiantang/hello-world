...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
	...
	@Override
	public String getOrgLength(String parentId){
		return sysDepartMapper.getOrgLength(parentId);
	}
...
/*	@Override
	...
	/*@Override
	@Transactional
	public void saveDepartData(SysDepart sysDepart, String username,String orgCodeValue) {
		if (sysDepart != null && username != null) {
			if (sysDepart.getParentId() == null) {
				sysDepart.setParentId("");
			}
			...
			//这里是框架自带的生成规则，需要修改成为自己的
			//根据parentId查询出来上级对应的orgCode然后把从前端数据字典中得到的编码拼接在orgCode后边即可
			String[] codeArray = (String[]) FillRuleUtil.executeRule(FillRuleConstant.DEPART,formData);
			//update-end--Author:baihailong  Date:20191209 for：部门编码规则生成器做成公用配置
			String parentOrgCode = sysDepartMapper.getParentOrgCode(parentId);

			String departName = null;
			if (parentOrgCode.length() == 9 || parentOrgCode.length() == 12){
				departName = orgCodeValue.substring(0,orgCodeValue.length() - 3);
				orgCodeValue = orgCodeValue.substring(orgCodeValue.length() - 3, orgCodeValue.length());
				sysDepart.setOrgCode(parentOrgCode + orgCodeValue);
...
/*	@Override
	@Transactional
	public void saveDepartData(SysDepart sysDepart, String username,String departID) {
		if (sysDepart != null && username != null) {
			if (sysDepart.getParentId() == null) {
				sysDepart.setParentId("");
			}
			...
			//根据departID查询出departName
			String departName = sysDepartMapper.getDepartNameById(departID);
			String CsCode = sysDepartMapper.getDepartNameCsCodeById(departID);

				// 更新部类、课类对应自个的部类课类编码
				if(CsCode.length() == 1){
					sysDepart.setCsBlCode(CsCode);
				}else if(CsCode.length() == 2){
					sysDepart.setCsKlCode(CsCode);
				}
				sysDepart.setOrgCode(parentOrgCode.substring(0,9) + departID);
				sysDepart.setDepartName(departName);
			}else {
				sysDepart.setOrgCode(codeArray[0]);
			}
		...
		if (sysDepart != null && username != null) {
			...
			if (parentId == null || "".equals(parentId)){
				...
				if (parentOrgCode.length() >= 9 ){
					...
					sysDepart.setOrgCode(parentOrgCode.substring(0,9) + departID);
					sysDepart.setDepartName(departName);
				}else if ("A01".equals(departID.substring(0,3))){
					sysDepart.setDepartName(departName);
					sysDepart.setOrgCode(departID);
				}else{
					sysDepart.setOrgCode(codeArray[0]);
				}
				String orgType = codeArray[1];
	...
	@Override
	public String getParentOrgCode(String pId) {
		return sysDepartMapper.getParentOrgCode(pId);
	}

	@Override
	public String getDepartName(String pId) {
		return sysDepartMapper.getDepartName(pId);
	}

	@Override
	public String getOrgCode(String chuName) {
		return sysDepartMapper.getOrgCode(chuName);
	}

	@Override
	public String getParentId(String chuName) {
		return sysDepartMapper.getParentId(chuName);
	}

	@Override
	public List<String> getIdList(List<String> idList) {
		return sysDepartMapper.getIdList(idList);
	}

	@Override
	public String getCaigouxianId(String s) {
		return sysDepartMapper.getCaigouxianId(s);
	}

	@Override
	public String getCodeById(String parentID) {
		return sysDepartMapper.getCodeById(parentID);
	}

	@Override
	public String getParentIdByCode(String parentID) {
		return sysDepartMapper.getParentIdByCode(parentID);
	}
}
