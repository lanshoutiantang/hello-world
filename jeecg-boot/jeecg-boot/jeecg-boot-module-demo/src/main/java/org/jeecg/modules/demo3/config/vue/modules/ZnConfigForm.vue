  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/config/znConfig/add",
          edit: "/config/znConfig/edit",
          queryById: "/config/znConfig/queryById"
        }
      }
    },
    computed: {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'shopid','classtype','orderdate','weekflag','safedays','arrivaldays','startweek','roundRule'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'shopid','classtype','orderdate','weekflag','safedays','arrivaldays','startweek','roundRule'))
      },
    }
  }
</script>