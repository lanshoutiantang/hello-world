...
public class ZnPromPlanController {
	...
	@GetMapping(value = "/list")
	 @PermissionData(pageComponent = "oademo3/intel/ZnPromPlanList")
	...
	public Result<?> edit(@RequestBody ZnPromPlanPage znPromPlanPage) {
	  ...
	  * @return
	  */
	 @AutoLog(value = "促销计划表-状态")
	 @ApiOperation(value="促销计划表-状态", notes="促销计划表-状态")
	 @PutMapping(value = "/checkStatus")
	 public Result<?> checkStatus(@RequestBody ZnPromPlanPage znPromPlanPage) throws Exception {
		 String planid = znPromPlanPage.getPlanid();
		 String status = znPromPlanPage.getStatus();
		 String id = znPromPlanPage.getId();
		 System.out.println(planid);
		 System.out.println(status);
		 System.out.println(id);
//		 Class.forName("com.mysql.cj.jdbc.Driver");
//		 Connection conn = DriverManager.getConnection("jdbc:mysql://192.168.6.168:3306/xinyangxiya?serverTimezone=Asia/Shanghai", "root", "hs845pehs845pe");
		 String sql = "call proc_prom_status('" + status + "','" + planid + "','" + id +"')";
    ...
    */
	...
	public ModelAndView exportXls(HttpServletRequest request, ZnPromPlan znPromPlan) {
	 ...
	 public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
		 // d是促销门店表导入的数据行数，b是促销门店表导入空值的数据行数，e是促销商品表导入的数据行数，d是促销商品表导入空值的数据行数
		 // cc是促销计划表导入的数据行数
		 int b=0;
		 int c=0;
		 int d=0;
		 ...
		 for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			 ...
			 try {
				 ...
				 for (ZnPromPlanPage page : list) {
					 ...
					 cc = cc+1;
					 List<ZnPromShop> list1 = new ArrayList();
					 List<ZnPromShop> list2 = new ArrayList();
					 List<ZnPromGoods> list3 = new ArrayList();
					 List<ZnPromGoods> list4 = new ArrayList();
					 list1 = page.getZnPromShopList();
					 list3 = page.getZnPromGoodsList();

					 for(int i = 0 ; i < list1.size() ; i++) {
						 if (list1.get(i).getShopid() != null) {
					 ...
					 for(int i = 0 ; i < list3.size() ; i++) {
						 if (list3.get(i).getGoodsid() != null) {
					 ...
					 znPromPlanService.saveMain(po, list2,list4);
				 }
				 return Result.OK("文件导入成功！促销计划表导入的数据行数:" + cc +";促销门店表导入的数据行数:" + d +",空值的数据行数:" +b +";促销商品表导入的数据行数:"+ e +",空值的数据行数:" +c,list.size());
			 } catch (Exception f) {
				 log.error(f.getMessage(),f);
				 return Result.error("文件导入失败:"+f.getMessage());
			 } finally {
				 try {
	  ...
	  * @param znPromPlan
	  */
	 @RequestMapping(value = "/moBanExportXls")
	 @PermissionData(pageComponent = "oademo3/intel/ZnPromPlanList")
	 public ModelAndView moBanExportXls(HttpServletRequest request, ZnPromPlan znPromPlan) {
		 // Step.1 组装查询条件查询数据
		 QueryWrapper<ZnPromPlan> queryWrapper = QueryGenerator.initQueryWrapper(znPromPlan, request.getParameterMap());
		 LoginUser sysUser = (LoginUser) SecurityUtils.getSubject().getPrincipal();
		 ...
		 if(oConvertUtils.isEmpty(selections)) {
			 queryList.clear();
			 znPromPlanList = queryList;
			 System.out.println(znPromPlanList);
			 a = 1;
		 }else {
			 queryList.clear();
			 znPromPlanList = queryList;
		 ...
		 if ( a == 1) {
			 ZnPromPlanPage vo = new ZnPromPlanPage();
			 vo.setPlanid("请输入促销计划编号 注：唯一值");
			 vo.setPlanname("促销计划名称");
			 Date data = new Date();
			 vo.setBegindate(data);
			 vo.setEnddate(data);
			 List<ZnPromShop> znPromShopList = new ArrayList();
			 ZnPromShop znPromShop1 = null;
			 znPromShop1 = new ZnPromShop();
			 znPromShop1.setShopid("注：唯一值");
			 znPromShopList.add(znPromShop1);
			 vo.setZnPromShopList(znPromShopList);
			 List<ZnPromGoods> znPromGoodsList = new ArrayList();
			 ZnPromGoods znPromGoods1 = null;
			 znPromGoods1 = new ZnPromGoods();
			 znPromGoods1.setGoodsid("注：唯一值");
			 double b=55.55;
			 BigDecimal decimalB=new BigDecimal(b);
			 znPromGoods1.setPrice(b);
			 znPromGoodsList.add(znPromGoods1);
			 vo.setZnPromGoodsList(znPromGoodsList);
			 pageList.add(vo);
