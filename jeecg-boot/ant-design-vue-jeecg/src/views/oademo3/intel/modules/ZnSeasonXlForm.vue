    ...
    <j-form-container :disabled="formDisabled">
      <a-form :form="form" slot="detail">
        <a-row>
          ...
          <a-col :span="24">
            <a-form-item v-has="'jie:bukejian'" label="季节性商品起季是首次到货日期" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="['arrdate']" placeholder="请输入季节性商品起季是首次到货日期"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="24">
            <a-form-item v-has="'jie:bukejian'" label="季节时间达到Period1，日均销量的折扣比例" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input-number v-decorator="['rate1']" placeholder="请输入季节时间达到Period1，日均销量的折扣比例" style="width: 100%"/>
            </a-form-item>
          </a-col>
          <a-col :span="24">
            <a-form-item v-has="'jie:bukejian'" label="季节时间达到Period2，日均销量的折扣比例" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input-number v-decorator="['rate2']" placeholder="请输入季节时间达到Period2，日均销量的折扣比例" style="width: 100%"/>
            </a-form-item>
          </a-col>
          <a-col :span="24">
            <a-form-item v-has="'jie:bukejian'" label="季节时间达到的比例2" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input-number v-decorator="['period2']" placeholder="请输入季节时间达到的比例2" style="width: 100%"/>
            </a-form-item>
          </a-col>
          <a-col :span="24">
            <a-form-item  v-has="'jie:bukejian'" label="季节时间达到的比例1" :labelCol="labelCol" :wrapperCol="wrapperCol">
  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        // this.$nextTick(() => {
        //   this.form.setFieldsValue(pick(this.model,'ywqid','xlid','xlname','begindate','enddate','arrdate','rate1','rate2','period1','period2','sdate','blOrganAll','klOrganAll'))
        // })
          this.form.setFieldsValue(pick(this.model.row,'ywqid','xlid','xlname','begindate','enddate','arrdate','rate1','rate2','period1','period2','sdate','blOrganAll','klOrganAll'))
      },
      //渲染流程表单数据
      showFlowData(){
        if(this.formBpm === true){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'ywqid','xlid','xlname','begindate','enddate','arrdate','rate1','rate2','period1','period2','sdate','blOrganAll','klOrganAll'))
      },
    }
  }
</script>