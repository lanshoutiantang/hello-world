  ...
  <a-modal
    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="名称编码" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input v-decorator="[ 'cgCsCode', validatorRules.cgCsCode]" placeholder="请输入名称编码"></a-input>
        </a-form-item>
  ...
  export default {
      ...
      return {
        ...
        validatorRules:{
          ...
          name:{rules: [{ required: true, message: '请输入类型名称!' }]},
            cgCsCode:{rules: [{ required: true, message: '请输入名称编码!' }]},
        },
        url: {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'pid','name','code','cgCsCode'))
        })
      },
      close () {
        this.$emit('close');
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'pid','name','code','cgCsCode'))
      },
